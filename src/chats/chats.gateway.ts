...
export class ChatsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async joinConversation(
    ...
    try {
      ...
      const conversation = await this.chatsService.getConversationForUser(
        userId,
  ...
  async typingStart(
    ...
    const context = await this.chatsService.getConversationPresenceForUser(
      userId,
      payload.conversationId,
    );
    const room = `conversation:${payload.conversationId}`;
  ...
  async markRead(
    ...
    try {
      result = await this.chatsService.markConversationAsRead(
        userId,
        payload.conversationId,
      );
    } catch (error) {
